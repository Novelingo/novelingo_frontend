...
export default function Footer() {
  let Links = [
    ...
    { name: "Generate a novel", link: "/generate/generate1" },
    { name: "Contact", link: "/" },
  ];
  return (
    <footer className="flex justify-around items-center md:px-20 px-5 md:py-20 py-10 flex-wrap">
      <div className="p-4">
        ...
        <p className="w-36 text-center text-light mt-4">
          Where Novels Shape Language Mastery!
        </p>
      </div>

      <div className="text-center underline underline-offset-2 flex flex-col p-4 text-white">
        {Links.map((link) => (
          <Link key={link.name} to={link.link} className="mt-4">
            {link.name}
          </Link>
        ))}
      </div>
      ...
      <div className="p-4">
        <div className="text-light flex flex-col">
          ...
          <span className="flex items-center">
            ...
            <p className="ml-2">Baku, Azerbaijan</p>
          </span>
        </div>
